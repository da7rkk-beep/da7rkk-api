# app.py - نسخة Flask كاملة للعمل على Vercel
...
# ============== دوال فودافون (الاتصالات الخارجية) ==============
...
.tab{padding:12px 25px;cursor:pointer;background:none;border:none;font-size:16px;color:#666;transition:all 0.3s}
...
.product-card{background:#f8f9fa;border-radius:12px;padding:15px;cursor:pointer;border:2px solid transparent;text-align:center;transition:all 0.3s}
...
<div class="form-group"><label>🔐 الرقم السري</label><input type="password" id="pin" maxlength="6" placeholder="أدخل الرقم السري (6 أرقام)" required></div>
...
<script>
const newProducts = [''' + ','.join([f'"{p}"' for p in NEW_PRODUCTS]) + '''];

const oldProducts = [''' + ','.join([f'"{p}"' for p in OLD_PRODUCTS]) + '''];
...
function displayProducts(){
    ...
    newProducts.forEach(p => {
        newC.appendChild(createCard(p, "جديد"));
    });
    
    oldProducts.forEach(p => {
        const type = p.includes("Mared") ? "مارد" : "قديم";
        oldC.appendChild(createCard(p, type));
    });
}
...
function createCard(product, type){
    ...
    div.innerHTML = '<div class="product-name">' + product + '</div><div class="product-type">' + type + '</div>';
    div.onclick = function(){
        document.querySelectorAll(".product-card").forEach(c => c.classList.remove("selected"));
        div.classList.add("selected");
        selectedProduct = product;
        document.getElementById("selectedProduct").value = product;
        document.getElementById("selectedDisplay").innerHTML = "✅ الكارت المختار: " + product;
    };
    return div;
}
...
function showResult(msg, type){
    ...
    if(type !== "info"){
        setTimeout(function(){ r.style.display = "none"; }, 8000);
    }
}
...
document.getElementById("purchaseForm").addEventListener("submit", async function(e){
    ...
    if(!selectedProduct){
        showResult("❌ الرجاء اختيار كارت أولاً", "error");
        return;
    }
    ...
    if(!receiver.match(/^01[0-9]{9}$/)){
        showResult("❌ رقم المستلم يجب أن يكون 11 رقماً ويبدأ بـ 01", "error");
        return;
    }
    
    if(!pin.match(/^[0-9]{6}$/)){
        showResult("❌ الرقم السري يجب أن يكون 6 أرقام", "error");
        return;
    }
    
    showResult("⏳ جاري تنفيذ العملية...", "info");
    ...
    try{
        ...
        if(data.success){
            showResult("✅ " + data.message + "<br>📱 الشحن للرقم: " + (data.receiver || receiver), "success");
            document.getElementById("pin").value = "";
        } else {
            showResult("❌ " + data.message, "error");
        }
    } catch(e){
        showResult("❌ خطأ في الاتصال بالخادم: " + e.message, "error");
    }
});
...
def health():
    return jsonify({"status": "healthy", "service": "dar7kk API"})
